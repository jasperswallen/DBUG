...
char *_db_process_ = (char *)"dbug"; /* Pointer to process name; argv[0] */
...
pthread_mutex_t THR_my_pthread_mutex_lock_dbug;

static void init_dbug_state(void) {
    pthread_mutex_init(&THR_my_pthread_mutex_lock_dbug, MY_MUTEX_INIT_FAST);
}
...
#define pthread_mutex_unlock(A)                                                \
    {}
static CODE_STATE static_code_state = {0,    0,    "?func", "?file", NULL, 0,
                                       NULL, NULL, 0,       "?",     0};
#endif
...
void _db_return_(uint _line_, const char **_sfunc_, const char **_sfile_,
                 uint *_slevel_) {
    CODE_STATE *state;
...
static BOOLEAN DoTrace(CODE_STATE *state) {
    ...
    if (TRACING && state->level <= stack->maxdepth &&
...
static BOOLEAN DoProfile() {
    ...
    profile = FALSE;
    if (PROFILING && state->level <= stack->maxdepth &&
...
_db_keyword_(const char *keyword) {
    ...
    state = code_state();
    result = FALSE;
    if (DEBUGGING && state->level <= stack->maxdepth &&
        InList(stack->functions, state->func) &&
...
static int vfprintf(stream, format, ap) FILE *stream;
char *format;
va_list ap;
{
    int rtnval;
